...
from span_api_splitter import config, logger, literals
...
def publish_splitting_service_out_message(payload, project_id, topic_id, publisher):
    ...
    logger.debug(
        f"audio splitting out message published successfully, result: {result}"
    )
    ...
    def process_message(self, data) -> None:
        try:
            ...
            if data and len(data) > literals.ZERO:
                ...
                logger.debug(
                    f"got result for {payload.get(literals.RESULT_ID)}, execution time: {process_time}s, points: "
...
def close_main_worker(signum, frame):
...
if __name__ == "__main__":
    ...
    worker = MainPubSubWorker(message_processor.process_message)
    signal.signal(signal.SIGTERM, close_main_worker)
    signal.signal(signal.SIGINT, close_main_worker)
    worker.run()
